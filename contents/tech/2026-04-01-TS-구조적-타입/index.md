...
* **비동기 I/O 최적화**: 수많은 외부 API와 통신하는 프록시 서버의 특성상, 이벤트 루프 기반으로 논블로킹 I/O를 처리하는 Node.js가 가장 적합했습니다.
* **구조적 유연성**: LLM 시장은 매주 새로운 모델과 응답 구조가 나올 정도로 변화가 빠릅니다. TypeScript는 구조적 타이핑을 통해 이러한 외부 의존성에 유연하게 대처하면서도 개발자가 정의한 최소한의 안정성을 확보할 수 있도록 합니다.
